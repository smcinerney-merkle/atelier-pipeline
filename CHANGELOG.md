...
### Fixed
- **`enforce-git.sh` union guard (G-147).** `source/claude/hooks/enforce-git.sh` and its `.claude/hooks/` copy now carry the union of this fork's guard and the-guru's: the global-option bypass narrowing (`-C`, `-c user.name=x`, and similar flag groups no longer let a blocked verb slip through for a SINGLE-TOKEN argument — a shell word mixing quoted and unquoted text with no space between them was a separate gap narrowed, but not fully closed, by the named-global-option entry below; see Known Issues for the two residual quoting/newline bypasses that remain), the eight added blocked verbs (`stash`, `revert`, `rm`, `cherry-pick`, `rebase`, `apply`, `am`, `merge`) behind a `([^-[:alnum:]]|$)` verb-group boundary, the `then|do|else|elif` prefix so a blocked verb after one of those four keywords is still caught, and the fork's own `\(` subshell anchor kept intact. The test-runner block (`colby`/`investigator`/`poirot`/main-thread allowlist and the BLOCKED message) is byte-identical to the pre-release baseline — untouched by this change.
- **`enforce-git.sh` named global-option list (G-147, post-blind-review, four rounds).** The `-flag [single-non-flag-token]` option group between `git` and the verb, in both `GIT_WRITE_EXEMPT` and the write-verb match, is replaced with `GIT_GLOBAL_OPT`. The generic form let a write verb hide inside an unrelated flag's "argument" (`git --no-pager merge --no-ff -m stash list` wrongly matched the `stash list` exemption and made a real merge commit) and, in the other direction, let a read-only subcommand's own argument collide with a blocked-verb spelling (`git --no-pager grep -n commit` was wrongly BLOCKED). A first pass replaced it with a full by-name enumeration of git's global options (`git help git`), split into argument-taking and no-argument forms — this by-name list was itself checked against real git 2.54.0 and found incomplete (`--no-advice`, `--no-lazy-fetch`, `--attr-source` were all absent, and the two argument-taking alternatives only consumed a single non-space token, so a shell-quoted argument with an embedded space split across the option boundary). A second pass named only the options confirmed to take a SEPARATE argument in real git (`-C`, `-c`, `--git-dir`, `--work-tree`, `--namespace`, `--attr-source`, `--config-env`) and treated every other dash-leading token generically as a no-argument flag (`-[^[:space:]]+`) — so an unnamed or future no-argument global option can no longer smuggle a verb past this hook by omission. `--config-env` was added in a third post-blind-review pass after its SPACE-argument form (not the already-caught `=` form) was confirmed live to slip a write verb past the guard unblocked; `--exec-path` is deliberately still not named — without a following `=` it prints the configured exec-path and exits without running any subcommand at all, so there is no verb position left for a write op to hide behind. `--super-prefix` is dropped entirely: it no longer exists in git 2.54. A fourth post-blind-review pass replaced each named option's argument (through the second/third passes: a double-quoted string, a single-quoted string, or a bare non-space token, matched only when the quote character sat at the START of the argument) with `GIT_SHELL_WORD`, a shared shell-word grammar — a run of one or more of a double-quoted segment, a single-quoted segment, a backslash-escaped character, or any other non-space/non-quote/non-backslash character — so a quote appearing ANYWHERE inside one shell word (`user.name="Sean M"`, `my" "dir`, not just a token that starts with a quote) is now consumed as part of the same argument — narrowing, not fully closing, the quoted-argument bypass: an escaped quote inside a quoted run and a literal newline inside a quoted value still slip past GIT_SHELL_WORD unblocked (see Known Issues, R1/R2). Confirmed live before this pass: `git -c user.name="Sean M" commit -m x`, `git -c user.name="A B" push`, `git -c user.name='A B' commit`, `git -C my\ dir commit`, `git -C a"b c" commit`, and `git --git-dir=my" "dir commit` all returned rc 0 (wrongly allowed; the first made a real commit in a scratch repo). `GIT_WRITE_EXEMPT`'s trailing-argument check is also switched from a blocklist (excluding a fixed set of shell metacharacters as the first character only) to a positive character class (`[A-Za-z0-9_/~^@:+=]` leading, `-` and, as of the second pass, `.`, and as of the fourth pass `{`/`}`, also allowed after the first character); the blocklist missed quotes, backslashes, and braces, so `checkout --detach "-f"` and similar quoted/escaped forms were wrongly exempted and discarded uncommitted changes. Excluding `.` from every position (first pass) also wrongly over-blocked dotted refs on an already-exempt subcommand (`merge --ff-only v5.2.2`, `checkout --track origin/v5.2.3`); `.` is now allowed after the first character only, so a bare `.` / leading-dot token stays correctly non-exempt. The fourth pass also found the continuation class excluded `{`/`}`, over-blocking a legitimate reflog-style ref on an already-exempt subcommand (`git stash show stash@{1}` returned rc 2, wrongly BLOCKED); `{`/`}` are now allowed after the first character only, so a token that OPENS with a brace is still correctly non-exempt.
- **SIGPIPE boot-corruption on large state files.** `session-boot.sh` and four Claude-only callers (`enforce-sequencing.sh`, `prompt-compact-advisory.sh`, `enforce-scout-swarm.sh`, `enforce-pipeline-activation.sh`) read `hook_lib_pipeline_status_field` through a pipe (`cat FILE | hook_lib_pipeline_status_field <field>`). On a state file large enough to fill the pipe buffer, the reading function's early exit sends SIGPIPE back to `cat`, and the shell's `|| echo "idle"` fallback fired alongside the real value, corrupting the parsed phase (observed: `'build\nidle'` instead of `'build'`). All call sites now use redirection (`hook_lib_pipeline_status_field <field> < FILE`) instead of a pipe. `session-boot.sh`'s unused `INPUT=$(cat 2>/dev/null) || true` (SC2034) is replaced with a bare, commented stdin drain.
- **`enforce-spawn-name.sh` SC2207.** The `PERSONAS=($(hook_lib_agent_personas))` array assignment is replaced with `IFS=' ' read -r -a PERSONAS <<< "$(hook_lib_agent_personas)"` for bash 3.2 compatibility (no `mapfile`/`readarray`) with the same word-splitting result.
- **`hooks.md` template hook-type registrations.** `prompt-brain-capture-reminder.sh`, `prompt-brain-prefetch.sh`, and `prompt-compact-advisory.sh` were registered as `"type": "prompt"` in the `pipeline-setup` template, which expects a literal prompt string rather than a command path — the same defect v5.1.8 already fixed in the live `.claude/settings.json` but never carried into the template. Flipped to `"type": "command"`. Each hook's existing `if` conditional is left exactly as written. `prompt-eva-path-reminder.sh` was also missing from the template entirely; added as `type: command`, positioned before `enforce-eva-paths.sh` in the `Write|Edit|MultiEdit` matcher, matching the live installed order.

### Changed
- **Pipeline model IDs updated for the Claude 5 family.** `pipeline-models.md`'s Provider-Shaped Model ID table moves `opus` and `sonnet` from the retired dated Bedrock/Vertex IDs to the bare aliases `claude-opus-5` / `claude-sonnet-5` (and provider-prefixed equivalents). Haiku is unchanged. The `synthesis` and `scout` rows are reworded to say their frontmatter declares the bare logical alias directly, rather than describing a pinned ID.
- **Git-write policy: three narrow allowances added.** `GIT_WRITE_EXEMPT` now permits `git merge --ff-only`, `git stash list` / `git stash show`, and `git checkout --detach` / `--track` / `--help` (bare, no destructive target) for non-Ellis agents — these are non-destructive branch/inspection operations, not writes in the sense the guard exists to block. Everything else the guard already blocked is still blocked.
- **Version bump 5.2.2 → 5.2.3** in all four plugin manifests: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.cursor-plugin/plugin.json`, `.cursor-plugin/marketplace.json`. `.claude/.atelier-version` deliberately stays at `5.1.6` — it records the installed version, and the plugin update process sets it, not this commit.

### Added
- **`tests/hooks/test_enforce_git_g147_replay.py`** — 111-case replay suite for the merged `enforce-git.sh` guard across colby/ellis/named-ellis: the original 33-case union replay, plus 25 cases added post-blind-review for the first named-global-option fix (F1 quoted/escaped trailing-argument bypasses, F2 verb-swallowed-as-flag-argument bypasses, F3 read-only-subcommand-argument false positives), plus 22 cases added in a second post-blind-review pass (G1 incomplete-option-list and quoted-argument bypasses, G2 over-blocked dotted-ref false positives), plus 4 cases added in a third post-blind-review pass (`--config-env` named-option gap), plus 21 cases added in a fourth post-blind-review pass (H1 mixed-quoting shell-word bypasses, H3 brace-in-reflog-ref over-blocking, plus two read-only option-group controls), plus 6 characterization cases added in a fifth pass (R1 escaped-quote-in-quoted-run leak, R2 literal-newline-in-quoted-value leak — both documented, current leaks per operator decision, not fixed).
- **`tests/hooks/test_enforce_git_guru_parity.py`** — 271-case ported parity suite covering the guard union (global-option bypass, added verbs, conditional prefixes, exemptions).
- **`tests/hooks/test_sigpipe_regression.py`** — 8-case suite proving the SIGPIPE corruption on the pre-fix baseline, its absence on the fix, and a structural guard against reintroducing the pipe form.

### Known Issues
- **Dead `if` conditionals remain on two Agent-matcher hook registrations.** `enforce-scout-swarm.sh` and `prompt-brain-prefetch.sh` register against an inert JS-expression `if` that never evaluates as intended (G-144); this is an accepted operator decision, not fixed here.
- **`git switch --discard-changes` still leaks.** The `enforce-git.sh` checkout regex only ever inspects the `checkout` verb, never `switch`; a documented, still-open gap.
- **A shell keyword other than `then`/`do`/`else`/`elif` still defeats the command-position anchor.** `enforce-git.sh`'s conditional prefix only tolerates those four keywords immediately before `git`; a bare `if`, `while`, `!`, a `{ ...; }` group, `time`, or a leading `VAR=val` assignment is not covered, so `if git commit -m x; then :; fi`, `while git push; do :; done`, `! git push`, `{ git push; }`, `time git push`, and `X=1 git push` all still leak past the write-verb guard. Documented, still-open gap; not fixed here.
- **A git alias still leaks past the write-verb guard.** `enforce-git.sh` matches literal verb spellings only; `git -c alias.ci=commit ci -m x` defines `ci` as an alias for `commit` on the same command line and returns rc 0 for a non-Ellis agent — the alias name never appears in the blocked-verb list, so there is no spelling to match against. Same indirection class as the existing `bash -c` / wrapper-script gap noted at the bottom of the hook file (both defeat the mechanical backstop, not the behavioral rule in `pipeline-orchestration.md`); not fixed here — the guard blocks named git operations, not arbitrary alias indirection.
- **An escaped quote inside a quoted `GIT_SHELL_WORD` run still leaks (R1).** `git -c user.name="A \" B" commit -m x`, `git -C "a \" b" commit`, and `git -c $'user.name=O\'Brien X' commit` all return rc 0 for colby — the double-quoted alternative (`"[^"]*"`) stops at the first unescaped `"`, so it cannot parse an escaped quote embedded inside a quoted run, and the generic non-space-run alternative that absorbs the leftover still stops short of the verb. Not a regression: also returns rc 0 for colby against the unmodified 5.2.2 guard and against the-guru's guard. Operator decision 2026-09-25: documented, not fixed here.
- **A literal newline inside a quoted option value still leaks (R2).** `enforce-git.sh`'s matching is line-oriented throughout (see the GIT_WRITE_EXEMPT SINGLE-LINE-ONLY property in the hook's own comments), so a real newline embedded inside a quoted value is invisible to the regex; `git -c user.name="A<LF>B" commit -m x` (an actual newline in the command string), `git -C "a<LF>b" push`, and `git -c "x.y=1<LF>" commit` all return rc 0 for colby. Not a regression: also returns rc 0 for colby against the unmodified 5.2.2 guard and against the-guru's guard. Operator decision 2026-09-25: documented, not fixed here.
- **The `\(` subshell anchor false-positives inside quoted strings** (e.g. `echo "(git add later)"`) — pre-existing, shared by both source guards, not addressed by this release.
- **Stale model IDs remain outside `pipeline-models.md`.** `CHANGELOG.md`, `.cursor-plugin/rules/telemetry-metrics.mdc`, `source/cursor/agents/*.frontmatter.yml` (13 files), `source/shared/references/pipeline-phases.md`, `source/shared/references/telemetry-metrics.md`, `source/shared/references/invocation-templates.md`, `.claude/references/telemetry-metrics.md`, `.claude/references/pipeline-phases.md`, `.claude/references/invocation-templates.md`, `docs/guide/technical-reference.md`, `docs/pipeline/last-case-file.md`, `docs/architecture/ADR-0041-effort-per-agent-map.md`, `docs/architecture/ADR-0048-scout-synthesis-model-pinning.md`, and `docs/architecture/ADR-0047-opus47-pipeline-tuning.md` were deliberately left untouched by this release's model-ID pass.
