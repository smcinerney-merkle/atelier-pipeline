...
- **Brain context consumption.** Eva prefetches brain context, injects via `<brain-context>`. Captures are gated mechanically (ADR-0053): a SubagentStop hook marks a pending capture only for the agents in `enforce-brain-capture-pending.sh`'s allowlist; a PreToolUse hook on `Agent` blocks Eva's next invocation until she calls `agent_capture` with curated content; PostToolUse on `agent_capture` clears the marker. Tool access and pending-capture obligation are separate: any agent granted `agent_capture` may call it for its own findings, but only the allowlisted subset's stop creates the marker; Eva curates cross-cutting concerns and fills gaps. `clear-brain-capture-pending.sh` guards the marker so a subagent's own capture cannot clear Eva's pending obligation.
