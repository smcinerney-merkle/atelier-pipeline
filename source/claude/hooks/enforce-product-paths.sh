#!/bin/bash
# Per-agent path enforcement: Robert-spec
# PreToolUse hook on Write|Edit -- Robert-spec can only write to docs/product/
set -uo pipefail
...
# Self-gate: this hook enforces robert-spec's paths only. It is registered
# both in the agent's frontmatter and in settings.json (so it also sees every
# other agent and the main thread). `agent_type` holds the registered type for
# a plain subagent ("robert-spec") and the instance name for a named teammate
# spawn ("robert-spec-retro") -- see hook_lib_agent_type_matches in
# hook-lib.sh. Any other agent (including plain "robert", or Eva's main
# thread, where agent_type is empty) exits 0 here -- this guard is not theirs
# to enforce.
...
if declare -f hook_lib_agent_type_matches >/dev/null 2>&1; then
  hook_lib_agent_type_matches "$AGENT_TYPE" robert-spec || exit 0
else
  # hook-lib.sh failed to load -- fall back to exact match (fail-narrow:
  # named instances like "robert-spec-retro" won't match and this guard will
  ...
  echo "WARNING: enforce-product-paths.sh: hook-lib.sh unavailable -- falling back to exact-match agent_type check ('$AGENT_TYPE' vs 'robert-spec'). Named Agent-tool instances (e.g. robert-spec-retro) will NOT match and this guard will not fire for them until hook-lib.sh is restored." >&2
  case "$AGENT_TYPE" in
    robert-spec) ;;
    *) exit 0 ;;
  esac
...
# default-persona.md). Robert-spec may write here ONLY its own report files, as
# direct children of the state directory: last-spec-*.md.
# These prefixes are the ones Eva names in each invocation's <output> (the
# practice this was ported from); robert-spec.md's <output> contract names the
...
# so last-spec-*.md alone would admit a nested path such as
# last-spec-x/pipeline-state.md.
...
case "$FILE_PATH" in
  "$PIPELINE_STATE_DIR"/*)
    ...
    echo "BLOCKED: Robert-spec may write under $PIPELINE_STATE_DIR/ only its own reports ($PIPELINE_STATE_DIR/last-spec-*.md), as direct children. The rest of the state directory is Eva's. Attempted: $FILE_PATH" >&2
    exit 2
    ;;
esac
