#!/bin/bash
# Per-agent path enforcement: Sarah
# PreToolUse hook on Write|Edit -- Sarah can only write to docs/architecture/
set -uo pipefail
...
# Self-gate: this hook enforces Sarah's paths only. It is registered both in
...
# plain subagent ("sarah") and the instance name for a named teammate spawn
# ("sarah-titles") -- see hook_lib_agent_type_matches in hook-lib.sh. Any
...
if declare -f hook_lib_agent_type_matches >/dev/null 2>&1; then
  hook_lib_agent_type_matches "$AGENT_TYPE" sarah || exit 0
else
  # hook-lib.sh failed to load -- fall back to exact match (fail-narrow:
  # named instances like "sarah-titles" won't match and this guard will
  ...
  echo "WARNING: enforce-sarah-paths.sh: hook-lib.sh unavailable -- falling back to exact-match agent_type check ('$AGENT_TYPE' vs 'sarah'). Named Agent-tool instances (e.g. sarah-titles) will NOT match and this guard will not fire for them until hook-lib.sh is restored." >&2
  case "$AGENT_TYPE" in
    sarah) ;;
    *) exit 0 ;;
  esac
...
# default-persona.md). Sarah may write here ONLY its own report files, as
# direct children of the state directory: last-adr-*.md.
# These prefixes are the ones Eva names in each invocation's <output> (the
# practice this was ported from); sarah.md's <output> contract names the
...
# so last-adr-*.md alone would admit a nested path such as
# last-adr-x/pipeline-state.md.
...
case "$FILE_PATH" in
  "$PIPELINE_STATE_DIR"/*)
    ...
    echo "BLOCKED: Sarah may write under $PIPELINE_STATE_DIR/ only its own reports ($PIPELINE_STATE_DIR/last-adr-*.md), as direct children. The rest of the state directory is Eva's. Attempted: $FILE_PATH" >&2
    exit 2
    ;;
esac

case "$FILE_PATH" in docs/architecture/*) exit 0 ;; esac
echo "BLOCKED: Sarah can only write to docs/architecture/. Attempted: $FILE_PATH" >&2
exit 2
