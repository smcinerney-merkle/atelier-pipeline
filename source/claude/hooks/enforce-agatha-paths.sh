#!/bin/bash
# Per-agent path enforcement: Agatha
# PreToolUse hook on Write|Edit|MultiEdit -- Agatha can only write to docs/
set -uo pipefail
...
# Self-gate: this hook enforces Agatha's paths only. It is registered both in
...
# plain subagent ("agatha") and the instance name for a named teammate spawn
# ("agatha-guide") -- see hook_lib_agent_type_matches in hook-lib.sh. Any
...
if declare -f hook_lib_agent_type_matches >/dev/null 2>&1; then
  hook_lib_agent_type_matches "$AGENT_TYPE" agatha || exit 0
else
  # hook-lib.sh failed to load -- fall back to exact match (fail-narrow:
  # named instances like "agatha-guide" won't match and this guard will
  ...
  echo "WARNING: enforce-agatha-paths.sh: hook-lib.sh unavailable -- falling back to exact-match agent_type check ('$AGENT_TYPE' vs 'agatha'). Named Agent-tool instances (e.g. agatha-guide) will NOT match and this guard will not fire for them until hook-lib.sh is restored." >&2
  case "$AGENT_TYPE" in
    agatha) ;;
    *) exit 0 ;;
  esac
...
# default-persona.md). Agatha may write here ONLY its own report files, as
# direct children of the state directory: last-agatha-*.md.
# These prefixes are the ones Eva names in each invocation's <output> (the
# practice this was ported from); agatha.md's <output> contract names the
...
# so last-agatha-*.md alone would admit a nested path such as
# last-agatha-x/pipeline-state.md.
...
# LOAD-BEARING, NOT REDUNDANT: Agatha's docs/* allowlist below admits the
# whole state directory by itself. This deny-before-allow block is what
# keeps Eva's files closed to her, so it must stay ahead of the docs/* case
# -- otherwise any change to that allowlist can reopen the hole.
...
case "$FILE_PATH" in
  "$PIPELINE_STATE_DIR"/*)
    case "${FILE_PATH#"$PIPELINE_STATE_DIR"/}" in
      */*) ;;
      last-agatha-*.md) exit 0 ;;
    esac
    echo "BLOCKED: Agatha may write under $PIPELINE_STATE_DIR/ only its own reports ($PIPELINE_STATE_DIR/last-agatha-*.md), as direct children. The rest of the state directory is Eva's. Attempted: $FILE_PATH" >&2
    exit 2
    ;;
esac

case "$FILE_PATH" in docs/*) exit 0 ;; esac
echo "BLOCKED: Agatha can only write to docs/. Attempted: $FILE_PATH" >&2
exit 2
